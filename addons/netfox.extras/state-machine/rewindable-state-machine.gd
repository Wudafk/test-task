...
func InjectBeforeReady(diContainer: DIContainer):
	gameTime = diContainer.rootContainer.GetByClassType(GameTime)
	gameTime.after_tick_loop.connect(_after_tick_loop)
	
	for child in find_children("*", "RewindableState", false):
		if child.has_method("InjectBeforeReady"):
			child.InjectBeforeReady(diContainer)


var networkTime: Node:
	get:
		if gameTime:
			return gameTime
		return null#_networkTime#NetworkTime
...
func _notification(what: int):
	...
	if what == NOTIFICATION_ENTER_TREE:
		...
		#_networkTime.after_tick_loop.connect(_after_tick_loop)
	elif what == NOTIFICATION_EXIT_TREE:
		# Disconnect handlers
		gameTime.after_tick_loop.disconnect(_after_tick_loop)
...
func _after_tick_loop():
	if _state_object != _previous_state_object:
		...
		if _previous_state_object:
			_previous_state_object.display_exit(_state_object, gameTime.tick)
		_state_object.display_enter(_previous_state_object, gameTime.tick)
